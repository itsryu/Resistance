...
class TeamSelectionDialog(CustomDialog):
    def __init__(self, parent: tk.Tk, leader_id: int, mission_size: int, available_players_ids: List[int], callback: Callable[[List[int]], None]):
        super().__init__(parent, f"Seleção de Equipe - Jogador {leader_id}", f"Jogador {leader_id}, clique nos jogadores para selecionar {mission_size} para a missão.")
    ...
    def _create_player_selection_ui(self):
        ...
        for player_id in self.available_players_ids:
            button = Button(players_frame, text=f"JOGADOR {player_id}", font=FONT_DEFAULT, bg=BG_MEDIUM, fg=TEXT_PRIMARY, relief="raised", bd=2, command=lambda p_id=player_id: self._toggle_player_selection(p_id))
...
class YesNoDialog(CustomDialog):
    def __init__(self, parent: tk.Tk, player_id: int, question: str, callback: Callable[[bool], None]):
        ...
        yes_button = Button(button_frame, text="✅ SIM", font=FONT_DEFAULT, bg="#28a745", fg="white", relief="flat", command=lambda: self._respond(True), activebackground="#218838", activeforeground="white")
        ...
        no_button = Button(button_frame, text="❌ NÃO", font=FONT_DEFAULT, bg="#dc3545", fg="white", relief="flat", command=lambda: self._respond(False), activebackground="#c82333", activeforeground="white")
