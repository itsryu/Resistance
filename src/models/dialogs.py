...
class CustomDialog(tk.Toplevel):
    ...
    def __init__(self, parent: tk.Tk, title: str, message: str):
        ...
        # Removed redundant <Return> binding here, as it will be specific to TeamSelectionDialog entry or confirm button
...
class TeamSelectionDialog(CustomDialog):
    ...
    def __init__(self, parent: tk.Tk, leader_id: int, mission_size: int,
        ...
        # Atualiza a mensagem para refletir a nova forma de seleção
        ...
        self.player_buttons: Dict[int, tk.Button] = {} # Para rastrear os botões dos jogadores

        self._create_player_selection_ui()

        # Label para exibir jogadores selecionados e o limite
    ...
    def _toggle_player_selection(self, player_id: int):
        ...
        if player_id in self.selected_team_ids:
            ...
            if len(self.selected_team_ids) < self.mission_size:
                ...
                self.player_buttons[player_id].config(bg=TEXT_ACCENT, fg=BG_DARK) # Cor de destaque quando selecionado
            else:
                messagebox.showwarning("Atenção", f"Você já selecionou o máximo de {self.mission_size} jogadores para a missão.")
        
        self.selected_team_ids.sort() # Mantém a ordem para consistência
    ...
    # These handlers are for the "Confirm Team" button
    ...
    # These handlers are for the Player selection buttons
    def _on_enter_player_button(self, event):
        if event.widget['bg'] != TEXT_ACCENT: # Only change if not already selected
...
class YesNoDialog(CustomDialog):
    ...
    def __init__(self, parent: tk.Tk, player_id: int, question: str, callback: Callable[[bool], None]):
        ...
        yes_button = Button(button_frame, text="✅ SIM", font=FONT_DEFAULT,
                            ...
                            activebackground="#218838", activeforeground="white") # Darker green on hover
        ...
        no_button = Button(button_frame, text="❌ NÃO", font=FONT_DEFAULT,
                           ...
                           activebackground="#c82333", activeforeground="white") # Darker red on hover
